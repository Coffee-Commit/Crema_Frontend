...
> = (set, get) => ({
  ...
  toggleAudio: async () => {
    ...
    try {
      ...
      if (currentState.publisher) {
        ...
        logger.debug('Publisher 오디오 제어', { enabled: newAudioEnabled })
      }
  ...
  toggleVideo: async () => {
    ...
    try {
      ...
      if (currentState.publisher) {
        ...
        logger.debug('Publisher 비디오 제어', { enabled: newVideoEnabled })
      }
  ...
  toggleScreenShare: async () => {
    ...
    try {
      ...
      const { toggleScreenShare } = await import('../screenshare/screenShareService')
      
      await toggleScreenShare(currentState.publisher)
      
      // 새로운 스토어의 상태를 확인하여 Feature store 상태 동기화
      const { useScreenShareStore } = await import('../screenshare/model/screenShare.store')
      const screenShareState = useScreenShareStore.getState()
      
      // 상태 업데이트
      get().updateSettings({ screenSharing: screenShareState.isScreenSharing })
  ...
  selectDevice: async (kind: MediaDevice['kind'], deviceId: string) => {
    const currentState = get()
    const device = currentState.availableDevices.find(d => 
      d.kind === kind && d.deviceId === deviceId
    )
    ...
    try {
      ...
      if (currentState.publisher && (kind === 'audioinput' || kind === 'videoinput')) {
