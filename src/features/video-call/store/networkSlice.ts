...
> = (set, get) => ({
  ...
  updateQuality: (quality: NetworkQuality) => {
    ...
    if (
      ...
      quality.level < currentState.quality.level - 1
    ) {
...
export const getNetworkRecommendations = (
  ...
  if (quality.latency > 150) {
    recommendations.push(
      '지연시간이 높습니다. 네트워크 연결을 확인해주세요',
    )
  }

  if (quality.packetLoss > 0.02) {
    recommendations.push(
      '패킷 손실이 발생하고 있습니다. 안정적인 네트워크로 연결해주세요',
    )
  }

  if (quality.bandwidth.upload < 500) {
    recommendations.push(
      '업로드 대역폭이 부족합니다. 비디오 품질을 낮춰보세요',
    )
  }

  if (quality.bandwidth.download < 500) {
    recommendations.push(
      '다운로드 대역폭이 부족합니다. 다른 네트워크 사용을 줄여보세요',
    )
  }
