...
> = (set, get) => ({
  ...
  connect: async (sessionInfo: SessionInfo, username: string) => {
    ...
    try {
      ...
      logger.info('세션 연결 완료', { sessionId: sessionInfo.id, username })
  ...
  disconnect: async () => {
    ...
    if (currentState.status === 'idle' || currentState.status === 'disconnected') {
      logger.debug('이미 연결 해제 상태', { status: currentState.status })
      return
    }
