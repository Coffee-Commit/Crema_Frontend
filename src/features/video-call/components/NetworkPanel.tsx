...
import { useNetworkQuality, useNetworkMonitoring, useVideoCallActions } from '../store'
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="rounded-lg bg-[var(--color-gray-50)] p-3">
                <p className="font-medium text-[var(--color-label-subtle)]">지연시간</p>
              ...
              {/* 지터 */}
              <div className="rounded-lg bg-[var(--color-gray-50)] p-3">
                <p className="font-medium text-[var(--color-label-subtle)]">지터</p>
                <p className="font-label3-semibold text-[var(--color-label-strong)]">
                  {quality.jitter}ms
                </p>
              </div>
              ...
              {/* 패킷 손실 */}
              <div className="rounded-lg bg-[var(--color-gray-50)] p-3">
                <p className="font-medium text-[var(--color-label-subtle)]">패킷 손실</p>
                <p className="font-label3-semibold text-[var(--color-label-strong)]">
              ...
              {/* 품질 점수 */}
              <div className="rounded-lg bg-[var(--color-gray-50)] p-3">
                <p className="font-medium text-[var(--color-label-subtle)]">품질 점수</p>
                <p className="font-label3-semibold text-[var(--color-label-strong)]">
                  {quality.level}/4
                </p>
              </div>
            ...
            {/* 대역폭 정보 */}
            <div className="rounded-lg bg-[var(--color-gray-50)] p-3">
              <p className="font-medium mb-2 text-[var(--color-label-subtle)]">대역폭</p>
              <div className="flex justify-between text-sm">
                <div>
                  <p className="text-[var(--color-label-subtle)]">업로드</p>
                  <p className="font-medium text-[var(--color-label-strong)]">
                    {quality.bandwidth.upload} kbps
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[var(--color-label-subtle)]">다운로드</p>
                  <p className="font-medium text-[var(--color-label-strong)]">
        ...
        {quality && quality.level <= 2 && (
          <div className="mt-4 rounded-lg border-l-4 border-yellow-400 bg-yellow-50 p-3">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="font-medium text-yellow-800 text-sm">네트워크 상태 주의</h3>
                <div className="mt-1 text-sm text-yellow-700">
                  <ul className="list-disc list-inside space-y-1">
                    {quality.latency > 150 && (
                      <li>지연시간이 높습니다. 네트워크 연결을 확인해주세요.</li>
                    )}
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            <div className={`h-2 w-2 rounded-full ${
              monitoring ? 'animate-pulse bg-green-500' : 'bg-[var(--color-gray-300)]'
            }`} />
