...
export default function MaterialsPanel() {
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      const uploadResult = (response.data as any).data || response.data.result
  ...
  const handleDownload = async (material: SharedMaterial) => {
    try {
      const response = await api.get<ApiResponse<ImageUrlResponse>>('/api/images/url', {
        params: { imageKey: material.imageKey }
      })
      
      const urlResult = (response.data as any).data || response.data.result
      ...
      const errorMessage = error?.response?.data?.message || error.message || '다운로드에 실패했습니다.'
      alert(errorMessage)
    }
  }
  // 파일 확장자 기반 아이콘 반환
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto">
        ...
        {jobSeekerLoading ? (
          ...
          <div className="space-y-[var(--spacing-spacing-2xs)]">
            ...
            <div>
              ...
              <p className="font-caption text-[var(--color-label-default)] leading-relaxed">
      ...
      <div className="flex-1 p-[var(--spacing-spacing-md)]">
        <div className="flex items-center justify-between mb-[var(--spacing-spacing-xs)]">
        ...
        {isUploading && (
          <div className="mb-[var(--spacing-spacing-6xs)]">
            ...
            <div className="w-full bg-[var(--color-gray-200)] rounded-full h-1">
              <div 
                className="bg-[var(--color-fill-primary)] h-1 rounded-full transition-all duration-300"
        ...
        {/* 업로드 에러 메시지 */}
        {uploadError && (
          <div className="mb-[var(--spacing-spacing-6xs)] rounded-[var(--radius-sm)] bg-red-50 border border-red-200 p-[var(--spacing-spacing-2xs)]">
            <span className="font-caption text-xs text-red-600">{uploadError}</span>
          </div>
        )}
          ...
          {materialsLoading ? (
            <div className="flex items-center justify-center py-8">
              <div className="text-[var(--color-label-subtle)]">
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />
                  <span className="font-caption">자료 로딩 중...</span>
                </div>
              </div>
            </div>
          ) : sharedMaterials.length === 0 ? (
            <div className="flex items-center justify-center py-8">
              <div className="text-[var(--color-label-subtle)] text-center">
                <svg
                  width="48"
                  height="48"
                  viewBox="0 0 24 24"
                ...
                <p className="font-caption text-xs mt-1">파일을 업로드해보세요!</p>
              </div>
            </div>
          ) : (
            ...
            <div
              ...
              className="rounded-[var(--radius-sm)] border border-[var(--color-border-default)] p-[var(--spacing-spacing-2xs)] transition-all hover:bg-[var(--color-fill-secondary)] cursor-pointer"
            >
              <div className="flex items-start gap-[var(--spacing-spacing-7xs)]">
                {/* 파일 아이콘 */}
                <div className="flex-shrink-0 text-[var(--color-label-default)] mt-0.5">
                  {getFileIcon(material.fileName)}
                </div>
                
                <div className="flex-1 min-w-0">
                  {/* 파일명 & 크기 */}
                  <div className="flex items-center gap-[var(--spacing-spacing-7xs)] mb-[var(--spacing-spacing-8xs)]">
                    <h4 className="font-label4-medium text-[var(--color-label-strong)] truncate">
                      {material.fileName}
                    </h4>
                    <span className="font-caption rounded-[var(--radius-xs)] bg-[var(--color-fill-tertiary)] px-[var(--spacing-spacing-8xs)] py-[var(--spacing-spacing-9xs)] text-[var(--color-label-subtle)] flex-shrink-0">
