...
export default function ParticipantVideo({
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-[var(--color-gray-800)]">
        <div className="text-center">
          <div className="mx-auto mb-1 flex h-8 w-8 items-center justify-center rounded-full bg-[var(--color-gray-600)]">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="text-[var(--color-fill-white)]">
              <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4z" />
            </svg>
          </div>
          <p className="text-xs text-[var(--color-fill-white)]">{participant.nickname}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[var(--radius-sm)] bg-[var(--color-gray-800)] cursor-pointer transition-all hover:ring-2 hover:ring-[var(--color-fill-primary)] ${className}`}
      onClick={handleClick}
    >
      {/* 비디오 스트림 */}
      <div
