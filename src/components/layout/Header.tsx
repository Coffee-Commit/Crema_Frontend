  ...
  return (
    <header className="border-b border-[var(--color-border-light)] bg-[var(--color-bg-default)]">
      <div className="container flex h-14 items-center justify-between">
        ...
        {/* 로그인 여부에 따라 다르게 */}
        {isLoggedIn ? (
          <div className="spacing-3xs flex items-center">
            ...
            <button className="rounded-radius-12 spacing-4xs bg-green-400 py-[2px] text-xs text-white">
              루키
            </button>
            <button
              onClick={logout}
              className="ml-spacing-4xs text-xs text-[var(--color-label-subtle)] underline"
            >
              로그아웃
            </button>
          </div>
          ...
          <div className="gap-spacing-3xs flex items-center">
            <span className="rounded-radius-24 px-spacing-3xs font-caption1 bg-[var(--color-label-primary)] py-[4px] text-white">
              nnn명의 루키가 당신의 경험을 기다리고 있어요!
            </span>
            <button
              onClick={login}
              className="rounded-radius-8 px-spacing-4xs border border-[var(--color-border-light)] bg-white py-[4px] text-sm"
            >
              로그인
            </button>
          </div>
