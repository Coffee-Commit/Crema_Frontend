...
//       id: '3',
//       nickname: '나루토마키',
//       appliedAt: '25.09.10 오후 10:00',
//       preferredDate: '25.09.11',
//       preferredTime: '22:35~23:59',
//       status: 'accepted',
//     },
//     {
//       id: '4',
...
function parseStartEnd(
  ...
  try {
    const [yy, mm, dd] = dateStr.split('-').map((s) => s.trim())
...
export default function DashboardPage() {
  const { guideId } = useAuthStore()
  const [pendingReservations, setPendingReservations] = useState<
    Reservation[]
  >([])
  const [thisWeekReservations, setThisWeekReservations] = useState<
    Reservation[]
  >([])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const resPending = await api.get<{
          message: string
          data: Reservation[]
        }>(`/api/guides/me/reservations/pending`)
        setPendingReservations(resPending.data.data)

        const resThisWeek = await api.get<{
          message: string
          data: { content: Reservation[] }
        }>(`/api/guides/${guideId}/coffeechats/this-week`, {
          params: {
            page: 0,
            size: 50,
            sort: ['preferredDateOnly,asc'],
          },
        })
        setThisWeekReservations(resThisWeek.data.data.content)
      } catch (e) {
  ...
  const group = useMemo(() => {
    ...
    const confirmed = thisWeekReservations.filter(
    ...
    return { pending, scheduled, done }
  }, [pendingReservations, thisWeekReservations])
  ...
  }, [scheduleFilter, thisWeekReservations])
