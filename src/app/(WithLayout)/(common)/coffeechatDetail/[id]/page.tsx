...
type GuideCoffeeChatResponse = {
  ...
  tags: { id: number; guideId: number; hashTagName: string }[]
  reviewScore: number
  reviewCount: number
  experiences: {
    groups: {
...
export default function CoffeeChatDetailPage() {
  ...
  useEffect(() => {
    const fetchCoffeeChat = async () => {
      try {
        const res = await api.get(`/api/guides/${id}/coffeechats`)
    ...
    const fetchStats = async () => {
      try {
        const res = await api.get(
          `/api/guides/${id}/coffeechat-stats`,
        )
        setStats(res.data.data)
      } catch (err) {
        console.error('❌ Stats 조회 실패:', err)
      }
    }
    if (id) fetchStats()
  }, [id])
  ...
  useEffect(() => {
    const fetchExperienceEval = async () => {
      try {
        const res = await api.get(
          `/api/guides/${id}/experience-evaluations`,
        )
        const result: GuideExperienceEvaluationResponse[] =
          res.data.data ?? []
        if (result.length > 0) {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await api.get(
          `/api/guides/${id}/reviews?page=0&size=100`,
        )
        const result: GuideReviewResponse = res.data.data
        setReviews(result?.reviews ?? [])
      } catch (err) {
  ...
  // ✅ 배너 데이터
  const bannerData = {
    categories: data.tags.map((tag) => tag.hashTagName),
    title: data.title,
    rating: data.reviewScore,
    reviewCount: data.reviewCount,
    keywords: data.tags.map((tag) => tag.hashTagName),
  }
